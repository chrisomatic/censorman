...
inline Color get_pixel(Image* image, s32 x, s32 y)
{
    Color c = {};
...
{
    for(s32 i = 0; i < image->w*image->h; ++i)
    {
        s32 n = i*image->n;
...
Color get_blended_color(u8* data, Color c, f32 opacity)
{
    u8 r = data[0];
    u8 g = data[1];
    u8 b = data[2];
...
f32 calc_iou(Box* a, Box* b)
{
    ...
    s32 union_area = area1 + area2 - inter_area;
...
void transform_scramble(Image* image, Box r, u32 seed)
{
    ...
    s32 num_pixels = r.w*r.h;
    s32 *unprocessed = (s32 *)PUSH_ARRAY(scratch.arena, s32, num_pixels);
    s32 unprocessed_count = num_pixels;
    ...
    for(;;)
    {
        ...
        s32 idx1 = rand() % unprocessed_count;
        s32 idx2 = rand() % unprocessed_count;

        // swap two pixels

        s32 u1 = unprocessed[idx1];
        s32 u2 = unprocessed[idx2];

        s32 offset1 = image->w*image->n*(u1/r.w) + image->n*(u1%r.w);
        s32 offset2 = image->w*image->n*(u2/r.w) + image->n*(u2%r.w);
...
void transform_rotate_rgb24(const u8 *src, u8 *dst, s32 width, s32 height, s32 rotation)
{
    s32 row, col;
    s32 dst_width = width;
    s32 dst_height = height;
...
static inline void put_pixel(Image *img, s32 x, s32 y, Color c)
{
...
static inline Color transform_blend_color(Color a, Color b, f32 factor)
{
...
static inline void blend_pixel(Image* img, s32 px, s32 py, Color c, f32 opacity)
{
    ...
    f32 alpha = (c.a / 255.0f) * opacity;
    dst[0] = (u8)((1 - alpha) * dst[0] + alpha * c.r);
    dst[1] = (u8)((1 - alpha) * dst[1] + alpha * c.g);
    dst[2] = (u8)((1 - alpha) * dst[2] + alpha * c.b);
}

static inline void draw_vline(Image* img, s32 x, s32 y1, s32 y2, Color c, f32 opacity) {

    if (x < 0 || x >= img->w) return;
    if (y1 > y2) { s32 tmp = y1; y1 = y2; y2 = tmp; }
    if (y1 < 0) y1 = 0;
...
void transform_draw_circle(Image* image, u16 x, u16 y, u16 radius, Color c, b32 filled, f32 opacity)
{
...
void transform_pixelate(Image* image, Box r, f32 block_scale)
{
    ...
    s32 block_size = MIN(r.w, r.h)*block_scale;
    ...
    s32 num_blocks_x = ceil(r.w / (f32)block_size);
    s32 num_blocks_y = ceil(r.h / (f32)block_size);
    ...
    for(s32 y = 0; y < num_blocks_y; ++y)
    {
        for(s32 x = 0; x < num_blocks_x; ++x)
        {
            ...
            s32 offset_x = x == num_blocks_x - 1 ? block_size_x - (r.w % block_size_x) : 0;
            s32 offset_y = y == num_blocks_y - 1 ? block_size_y - (r.h % block_size_y) : 0;

            // apply avgcolor to range
            curr = start + y*block_size_y*step + x*block_size_x*n;

            for(s32 j = 0; j < block_size_y - offset_y; ++j)
            {
                for(s32 i = 0; i < block_size_x - offset_x; ++i)
                {
...
void transform_stretch_image(Image *dst, Image *src, Box r)
{
    ...
    for (s32 dy = 0; dy < r.h; ++dy)
    {
        for (s32 dx = 0; dx < r.w; ++dx)
        {
            ...
            s32 sx = (s32)(dx * scaleX);
            s32 sy = (s32)(dy * scaleY);
            ...
            b32 ignore = (src_pixel[0] == 0xFF && src_pixel[1] == 0x00 && src_pixel[2]);
...
f32 lanczos_table[KERNEL_TABLE_SIZE];
f32 inv_a_scale = 0.0;

// performs pre-computations to make things fast
void lanczos_init(s32 a) {
    for (s32 i = 0; i < KERNEL_TABLE_SIZE; ++i) {
        f32 x = ((f32)i / (KERNEL_TABLE_SIZE - 1)) * a;
        if (x == 0.0)
            lanczos_table[i] = 1.0;
        else if (x < a)
    ...
    inv_a_scale = (KERNEL_TABLE_SIZE - 1) / (f32)a;
}

static inline f32 fast_lanczos(f64 x, s32 a) {

    x = ABSF(x);
    if (x >= a)
        return 0.0;

    s32 idx = (s32)(x*inv_a_scale);
    return lanczos_table[idx];
}

void lanczos_downscale_rotate(Image *in, Image *out, s32 a)
{
    f64 x_scale, y_scale;
    ...
    switch (out->rotation)
    {
        case 90:
        case 270:
            x_scale = (f64)in->w / out->h;
            y_scale = (f64)in->h / out->w;
            break;
        default: // 0° or 180°
            x_scale = (f64)in->w / out->w;
            y_scale = (f64)in->h / out->h;
            break;
    }

    for (s32 oy = 0; oy < out->h; ++oy)
    {
        for (s32 ox = 0; ox < out->w; ++ox)
        {
            f64 fx, fy;
            ...
            s32 x_start = floor(fx - a);
            s32 x_end   = floor(fx + a);
            s32 y_start = floor(fy - a);
            s32 y_end   = floor(fy + a);

            f64 sum_r = 0.0, sum_g = 0.0, sum_b = 0.0;
            f64 sum_w = 0.0;

            for (s32 sy = y_start; sy <= y_end; ++sy)
            {
                s32 cy = sy < 0 ? 0 : (sy >= in->h ? in->h-1 : sy);
                f64 wy = fast_lanczos((fy - sy) / y_scale, a);

                for (s32 sx = x_start; sx <= x_end; ++sx)
                {
                    s32 cx = sx < 0 ? 0 : (sx >= in->w ? in->w-1 : sx);
                    f64 wx = fast_lanczos((fx - sx) / x_scale, a);
                    f64 w = wx * wy;
                    if (w == 0.0) continue;

                    s32 src_idx = (cy * in->w + cx) * in->n;
            ...
            s32 dst_idx = (oy * out->w + ox) * out->n;
            if (sum_w > 0.0)
            {
                out->data[dst_idx + 0] = (u8)(sum_r / sum_w + 0.5);
                out->data[dst_idx + 1] = (u8)(sum_g / sum_w + 0.5);
                out->data[dst_idx + 2] = (u8)(sum_b / sum_w + 0.5);
            }
            else
            {
...
void lanczos_downscale(Image *in, Image *out, s32 a)
{
    f64 x_scale = (f64)in->w / out->w;
    f64 y_scale = (f64)in->h / out->h;

    for (s32 y = 0; y < out->h; ++y)
    {
        f64 source_y = (y + 0.5) * y_scale;
        s32 y_start = floor(source_y - a);
        s32 y_end   = floor(source_y + a);

        for (s32 x = 0; x < out->w; ++x)
        {
            f64 source_x = (x + 0.5) * x_scale;
            s32 x_start = floor(source_x - a);
            s32 x_end   = floor(source_x + a);
            ...
            {
                f64 weight_y = fast_lanczos((j - source_y) / y_scale, a);
                s32 clamped_j = j < 0 ? 0 : (j >= in->h ? in->h-1 : j);
                f32 row_off = clamped_j*in->w*in->n;

                for (s32 i = x_start; i <= x_end; ++i)
                {
                    // Calculate weights using the Lanczos kernel
                    f64 weight_x = fast_lanczos((i - source_x) / x_scale, a);
                    f64 weight = weight_x * weight_y;

                    s32 clamped_i = i < 0 ? 0 : (i >= in->w ? in->w-1 : i);
                    s32 offset = row_off + clamped_i*in->n;
...
b32 transform_downscale(Arena* arena, Image* source, Image* result, s32 scaled_size, s32 rotation)
{
    ...
    if(use_scaled_image)
    {
        const s32 a = 2; // number of iterations
        lanczos_init(a);

        // downscale largest dimension 
        f32 aspect = source->w / (f32)source->h;
...
void transform_box_upscale_rotate_inverse(Box* r,u16 det_w, u16 det_h, u16 orig_w, u16 orig_h, s32 rotation)
{
    ...
    switch (rotation)
    {
        case 90:
        case 270:
            scale_x = (f32)orig_w / scaled_h;
            scale_y = (f32)orig_h / scaled_w;
            break;
        default: // 0 or 180
            scale_x = (f32)orig_w / scaled_w;
            scale_y = (f32)orig_h / scaled_h;
            break;
    }
...
static void generate_kernel(f32 sigma, f32 **kernel, s32 *k_size)
{
    s32 radius = (s32)ceilf(3 * sigma);
    *k_size = 2 * radius + 1;
    *kernel = (f32 *)malloc((*k_size) * sizeof(f32));
...
void transform_gaussian_blur(Image *image, Box *r)
{
    ...
    f32 base = (r->w < r->h ? r->w : r->h);
    f32 sigma = 0.24 * settings.blur_strength * base;   // tune multiplier to taste
...
void transform_box_blur(Image *image, Box *r, u8 *buffer)
{
    ...
    f32 base = (r->w < r->h ? r->w : r->h);
    s32 radius = (s32)(0.24f * settings.blur_strength * base);
    if (radius < 1) radius = 1;

    s32 k_size = 2 * radius + 1;
    f32 norm = 1.0f / k_size;

    // Precompute clamped indices for horizontal and vertical passes
    s32 *h_indices = (s32 *)malloc((r->w + 2 * radius) * sizeof(s32));
    s32 *v_indices = (s32 *)malloc((r->h + 2 * radius) * sizeof(s32));

    for (s32 i = -radius; i < r->w + radius; ++i)
    {
        s32 idx = r->x + i;
        if (idx < r->x) idx = r->x;
    ...
    for (s32 i = -radius; i < r->h + radius; ++i)
    {
        s32 idx = r->y + i;
        if (idx < r->y) idx = r->y;
    ...
    for (s32 pass = 0; pass < 3; ++pass)
    {
        ...
        for (s32 y = r->y; y < r->y + r->h; ++y)
        {
            ...
            for (s32 c = 0; c < image->n; ++c)
            {
                ...
                for (s32 x = r->x + 1; x < r->x + r->w; ++x)
                {
                    s32 left  = h_indices[x - r->x - 1 + 0]; // previous left
                    s32 right = h_indices[x - r->x + k_size - 1]; // new right
        ...
        for (s32 x = r->x; x < r->x + r->w; ++x)
        {
            for (s32 c = 0; c < image->n; ++c)
            {
                ...
                for (s32 y = r->y + 1; y < r->y + r->h; ++y)
                {
                    s32 top    = v_indices[y - r->y - 1 + 0];
                    s32 bottom = v_indices[y - r->y + k_size - 1];
